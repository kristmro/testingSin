...
class Reptile(Model):
    ...
    def copyWeightsApply(self, source, target, fn=lambda s, t: s):
        """Assign weights from source to target. Apply a transform fn to source and target weights."""
        for j in range(len(source.trainable_weights)):
            target.trainable_weights[j].assign(
                fn(tf.convert_to_tensor(source.trainable_weights[j]), 
                tf.convert_to_tensor(target.trainable_weights[j]))
            )
    ...
    def taskLoss(self, batch):
        ...
        for _ in range(self.nInnerSteps):
            ...
            gradients = taskTape.gradient(loss, self.modelCopy.trainable_variables)
            self.sgd.apply_gradients(zip(gradients, self.modelCopy.trainable_variables))
